...
module.exports = {
  ...
  stackHasRoom(stack) {
    ...
    //fixme: does not work. when plugin create nested stack outputs count every time = 0 but when deploy count greater 60
    //fixme: parameters count works similar
    const outputs = stack.Outputs || {};
    const parameters = stack.Parameters || {};
    console.log('params count: ', Object.keys(parameters).length)
// console.log('resources count: ', Object.keys(resources).length)
// console.log('outputs count: ', Object.keys(outputs).length)
    return Object.keys(resources).length < 55 && Object.keys(parameters).length < 60;
